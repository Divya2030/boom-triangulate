...
\textbf{BOOM}~\cite{boom} is an out-of-order RISC-V~\cite{riscv} core. A
...
The main limit is honest and simple. Single-core bare-metal tests cannot see
every kind of bug: three of our eleven planted bugs never change committed state
this way, so no comparison can catch them. Two of them are multi-core memory
effects that need more than one hart to show up. The vote also has one known
rough edge: the reference-model mismatch on the exception test is a logging
habit, and we should normalise it the same way we did for BOOM. Finally, these
are planted bugs that show the method works; we have not yet found a new,
real bug in BOOM.
